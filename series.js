...
//  ELEMENTOS
// ======================================================
const numero = document.getElementById("numero");   // agora representa a SÉRIE
...
function adicionarItemNaLista(obj, isFixo = false) {
    ...
    btnApagar.addEventListener("click", () => {
        mostrarPopupConfirmacao("Tem a certeza que deseja apagar esta série?")
            .then(confirmar => {
                if (!confirmar) return;
...
// ======================================================
//  VALIDAÇÕES (ADAPTADAS PARA SÉRIES)
// ======================================================
numero.addEventListener("input", () => {
    numero.value = numero.value.replace(/[^0-9]/g, "").slice(0, 2);
    if (numero.value.length === 2) valor.focus();
    mostrarTotalDoNumero();
    validarTudo();
});
...
function validarTudo() {
    ...
    if (numero.value.length !== 2)
        return erro("⚠️ A série deve ter 2 dígitos");
    ...
    if ((numValor * 100) % 250 !== 0)
        return erro("⚠️ O valor deve ser múltiplo de 2.50");

    const falta = 100 - totalDoNumero(numero.value);
    if (numValor > falta)
        return erro(`⚠️ Esta série só pode receber mais ${falta.toFixed(2)}`);
...
botao.addEventListener("click", e => {
    ...
    aviso.textContent = "✔️ Série registada com sucesso!";
...
apagarTudo.addEventListener("click", () => {
    mostrarPopupConfirmacao("Tem a certeza que deseja apagar TODAS as séries?")
        .then(confirmar => {
            if (!confirmar) return;
